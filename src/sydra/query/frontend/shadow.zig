...
pub fn parseSydraqlShadow(allocator: std.mem.Allocator, source: []const u8) !ShadowParseResult {
    ...
    var parser = legacy_parser.Parser.init(allocator, source);
...
test "shadow sydraql parser tracks insert and delete statement kinds" {
    ...
    defer alloc.free(insert_result.diagnostics);
    defer alloc.free(insert_result.emitted.emitted_source);
    ...
    defer alloc.free(delete_result.diagnostics);
