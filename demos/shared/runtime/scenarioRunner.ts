...
export type SummaryEvidenceBuilder = (result: ScenarioRunResult) => Record<string, unknown> | undefined;
...
async function executeStep(
  ...
  const workspace = step.kind === "server_control"
    ? sandbox.workspaces[step.workspace]
    : "workspace" in step
      ? sandbox.workspaces[step.workspace]
      : undefined;
  ...
  switch (step.kind) {
    ...
    case "wait": {
      ...
      output = { durationMs: step.durationMs, status: "settled" };
      break;
    }
    case "http_request": {
      const resolvedBody = step.body == null ? undefined : resolveTemplates(step.body, sandbox.vars);
      const resolvedPath = resolveTemplates(step.path, sandbox.vars);
      command = serializeCommand(step);
      await startWorkspaceServer(workspace!, binaryPath);
      output = await requestWorkspace(workspace!, {
        method: step.method,
        path: resolvedPath,
        body: resolvedBody as HttpRequestStep["body"],
...
export async function runScenarioManifest(
  ...
  result.summaryEvidence = summaryEvidenceBuilder?.(result);
  return result;
}
